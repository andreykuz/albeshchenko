...
--
-- Temporary table structure for view `view_people`
--

DROP TABLE IF EXISTS `view_people`;
/*!50001 DROP VIEW IF EXISTS `view_people`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE TABLE `view_people` (
  `idpeople` tinyint NOT NULL,
  `sename` tinyint NOT NULL,
  `name_` tinyint NOT NULL,
  `lastname` tinyint NOT NULL,
  `id_PGO` tinyint NOT NULL,
  `id_nPGO` tinyint NOT NULL,
  `stat` tinyint NOT NULL,
  `typereg` tinyint NOT NULL,
  `address` tinyint NOT NULL,
  `datebirth` tinyint NOT NULL,
...
--
-- Final view structure for view `view_people`
--

/*!50001 DROP TABLE IF EXISTS `view_people`*/;
/*!50001 DROP VIEW IF EXISTS `view_people`*/;
/*!50001 SET @saved_cs_client          = @@character_set_client */;
/*!50001 SET @saved_cs_results         = @@character_set_results */;
/*!50001 SET @saved_col_connection     = @@collation_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_people` AS select `people`.`idpeople` AS `idpeople`,`people`.`sename` AS `sename`,`people`.`name_` AS `name_`,`people`.`lastname` AS `lastname`,`people`.`id_PGO` AS `id_PGO`,`people`.`id_nPGO` AS `id_nPGO`,`people`.`stat` AS `stat`,`people`.`typeReg` AS `typereg`,`people`.`address` AS `address`,`people`.`dateBirth` AS `datebirth`,`directory_street`.`street_name` AS `street_name`,`directory_village`.`village_name` AS `village_name`,`pgo`.`num_house` AS `num_house`,`pgo`.`num_flat` AS `num_flat` from (((`people` join `pgo` on(((`pgo`.`idPGO` = `people`.`id_PGO`) and (`pgo`.`nPGO` = `people`.`id_nPGO`)))) join `directory_street` on((`pgo`.`idstreet` = `directory_street`.`id_street`))) join `directory_village` on((`pgo`.`idvillage` = `directory_village`.`id_village`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `view_pgo` AS select `pgo`.`idPGO` AS `idPGO`,`pgo`.`nPGO` AS `nPGO`,`directory_village`.`village_name` AS `village_name`,`directory_street`.`street_name` AS `street_name`,`pgo`.`num_house` AS `num_house`,`pgo`.`num_flat` AS `num_flat`,concat(`people`.`sename`,' ',`people`.`name_`,' ',`people`.`lastname`) AS `Власник`,`pgo`.`vukl` AS `vukl` from (((`pgo` join `directory_street` on((`pgo`.`idstreet` = `directory_street`.`id_street`))) join `directory_village` on((`pgo`.`idvillage` = `directory_village`.`id_village`))) left join `people` on(((`pgo`.`idPGO` = `people`.`id_PGO`) and (`pgo`.`nPGO` = `people`.`id_nPGO`)))) where ((`people`.`golovaGospod` <> 0) or isnull(`people`.`id_PGO`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2013-12-03  0:22:22
